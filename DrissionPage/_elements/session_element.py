...
class SessionElement(DrissionElement):
    ...
    def _get_ele_path(self, xpath = True):
        HtmlElement = self._inner_ele
        xpath_path = HtmlElement.getroottree().getpath(HtmlElement)
        return xpath_path if xpath else (xpath_path.lstrip('/').replace('[', ':nth-of-type(')
                                         .replace(']', ')').replace('/', '>'))
