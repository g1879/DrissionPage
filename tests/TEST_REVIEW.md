...
- 测试总体贴近真实：浏览器用例启动真实 Google Chrome，并通过本地 HTTP、WebSocket、SSE 和跨 host iframe fixture 驱动；源码测试中没有使用 `Mock`、`MagicMock` 或 pytest monkeypatch 替代核心浏览器行为。少数无浏览器用例使用手写 fake 验证调用形状，见下方限制。
- 本地 fixture 比第三方网站更适合作为主门禁：协议和浏览器交互是真实的，同时避免站点波动、反自动化和公网故障造成误报。公网 smoke 与独立 SSR test-site 保持 `local` 显式启用是合理的。
- 测试确实能发现实现问题。本次 `stable` 初始基线为纯 Python `6 passed / 2 failed`、真实 Chrome `18 passed / 4 failed`；失败分别落在根导出/启动兼容、AX domain 生命周期和相对 XPath 生成。
- 原分组已经失真：同一 API 缺陷同时被 `stable` 和 `known` 复现，导致“稳定门禁”天然失败。本次已将 6 个失败 case 临时隔离到 `known`，保留失败和 traceback，不用 skip 或删除掩盖问题。
- 旧 `known` 中有 4 个 current 用例单次通过，但没有 pre-release + Linux CI + 重复运行证据，暂不提升回主门禁。
- 独立 Astro fixture 复核为 2 通过 / 1 失败：Marketplace 与社区移动端通过；完整 `ssr_site_smoke` 在 AX 定位处复现相同 domain 生命周期缺陷，已从 CI 强制门禁拆为 report-only。

### 新鲜运行证据

| 批次 | 结果 | 说明 |
| --- | --- | --- |
| `stable --skip-browser --no-browser-only`（隔离前） | 6 通过 / 2 失败 | 失败：`feature_recommended_chromium_entry`、`feature_startup_settings`。 |
| `stable --browser-only`（隔离前） | 18 通过 / 4 失败 | 失败：`feature_element_locators`、`feature_element_core_behaviors`、`feature_find_api`、`locator_behavior`。 |
| `known --no-browser-only`（隔离前） | 0 通过 / 1 失败 | `api_surface` 同时覆盖多项公开 API 差异。 |
| 逐项 known 浏览器复现 | 4 通过 / 7 失败 | 每项使用独立进程上限；`alert_on_open` 由其内部 8 秒超时确认挂起。 |
| 本地 Astro SSR 三场景 | 2 通过 / 1 失败 | Marketplace、社区移动端通过；完整 smoke 失败于 AX domain 未启用。 |

复核环境为 macOS、Python 3.11.11、Google Chrome `149.0.7827.198`。调整后完整 `tests/ci.sh` 编排在 current 源码和独立 venv 中的 PyPI `5.0.0b0` 上退出 0：两边 stable 都是纯 Python 6/6、浏览器 18/18；known 与完整 SSR smoke 仍保留失败报告，SSR 业务门禁 2/2 通过。

线上 run `29393359977` 进一步确认：workflow checkout 的本地 SSR 业务流 2/2 通过，但 `DP_PRIVATE_FIXTURE_URL` 指向的远端部署对 `/api/marketplace/search.json` 和 `/api/social-notes/feed.json` 返回 404。远端部署版本漂移不属于 DrissionPage 主测试回归，因此远端 fixture 检查已改为 report-only。

### 真实性与盲区

| 维度 | 当前判断 |
| --- | --- |
| API/配置契约 | 真实且确定，但必须以明确兼容策略和 `.pyi`/文档为依据，避免把有意破坏性变更误判成 bug。 |
| 手写 fake 契约 | `mouse_trail` 和 `Actions.drag_in` 只证明 JS/CDP 调用链与参数形状，不能证明浏览器实际指针轨迹、拖拽事件或页面效果。 |
| DOM/导航/下载/cookie | 真实 Chrome + 本地服务，能够发现运行时和清理问题，是当前最可信的一层。 |
| Network/WS/SSE/iframe | 使用真实协议和 CDP 事件，已测出 packet shape、监听恢复和跨 iframe 捕获问题；仍缺乱序、断连和高并发矩阵。 |
| SSR 业务流 | 场景更接近产品页面，但依赖独立 test-site checkout，适合作为 CI 专项门禁，不应替代小型回归 fixture。 |
| 外部网站 smoke | 只适合 `local` 辅助证据，失败首先应归因环境/站点，不能直接判定 DrissionPage 回归。 |
| 平台兼容 | 当前主要覆盖 Chrome；Edge、不同 Chrome revision、Windows/Linux 差异仍不足。 |
| 长期稳定性 | 缺少资源泄漏、浏览器崩溃/重连、长连接、压力和竞态测试。 |

### 当前停止条件

本轮只调整测试分组与证据文档，不修库实现。后续每修复一个问题，应先用对应 `known` case 加 `--fail-on-failures` 证明，再补相邻契约与清理断言，最后完成 current/pre-release/Linux headless 矩阵后移回 `stable`。
