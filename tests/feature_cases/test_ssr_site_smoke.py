...
def run(ctx):
    ...
    if not base:
        ctx.skip(required_url_message('SSR fixture smoke'))
        return

    session_page = SessionPage()
    health = session_page.get(test_site_url('/api/health.json'), timeout=20)
    ...
    manifest = session_page.get(test_site_url('/api/manifest.json'), timeout=20)
    ...
    missing = session_page.get(test_site_url('/api/status/404'), timeout=20, retry=0, raise_err=False)
    assert_nav_result(missing, status=404, ok=False, label='SSR explicit 404 endpoint')

    dynamic = session_page.get(test_site_url('/cases/dynamic?name=session-smoke'), timeout=20)
    ...
    download = session_page.get(test_site_url('/api/download.txt?name=session-fixture.txt'), timeout=20)
    ...
    activity_batch = session_page.get(test_site_url('/api/activity-batch.json?offset=0&count=5&delay=1'), timeout=20)
    ...
    activity_limit = session_page.get(test_site_url('/api/activity-batch.json?offset=120&count=80&delay=1'), timeout=20)
    ...
    commerce_products = session_page.get(test_site_url('/api/commerce/products.json?offset=0&count=4&delay=1'), timeout=20)
    ...
    cf_blocked = session_page.get(test_site_url('/api/cf/protected.json'), timeout=20, retry=0, raise_err=False)
    ...
    cf_clearance = session_page.get(test_site_url('/cdn-cgi/challenge-platform/fixture-clearance'), timeout=20)
    ...
    cf_protected = session_page.get(test_site_url('/api/cf/protected.json'), timeout=20)
    ...
    cf_rate = session_page.get(test_site_url('/api/cf/protected.json?mode=rate-limit'), timeout=20, retry=0, raise_err=False)
    ...
    with make_browser(ctx.browser_path, page_load_timeout=20) as browser:
        ...
        locators = tab.get(test_site_url('/cases/locators'))
        ...
        network = tab.get(test_site_url('/cases/network'))
        ...
        forms = tab.get(test_site_url('/cases/forms'))
        ...
        frames = tab.get(test_site_url('/cases/frames'))
        ...
        waits = tab.get(test_site_url('/cases/waits'))
        ...
        visual = tab.get(test_site_url('/cases/visual'))
        ...
        upload_download = tab.get(test_site_url('/cases/upload-download'))
        ...
        business = tab.get(test_site_url('/cases/business-dashboard'))
        ...
        commerce = tab.get(test_site_url('/cases/commerce'))
        ...
        cf_page = tab.get(test_site_url('/cases/cloudflare-gate'))
