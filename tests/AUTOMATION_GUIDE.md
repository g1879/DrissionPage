...
9. 在非 pull request 事件中，如果配置了 `DP_PRIVATE_FIXTURE_URL`，以 report-only 方式执行远端 Marketplace、社区笔记移动端流程和完整 SSR 冒烟；远端部署版本或可用性不影响主门禁。
10. 上传测试报告和运行时产物。

`stable` 和 workflow checkout 的本地 SSR 业务场景失败会导致工作流失败。`known` 以及远端共享/私有 fixture 失败只写入报告，不改变工作流结果。
