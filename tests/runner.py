...
def build_report(results: list[TestResult], cases, started: float, ctx: TestContext) -> dict[str, Any]:
...
def write_markdown(report: dict[str, Any], path: Path) -> None:
    ...
    for case in report["cases"]:
        lines.append(f"- **{case['name']}** — `{case['status']}` ({case['duration_seconds']}s): {case['message'] or case['title']}")
    lines.append("")
...
def main(argv=None) -> int:
    ...
    if args.list_cases:
        for case in cases:
            print(f"{case.name}	{case_phase_label(case)}	{case.title}")
        return 0
    ...
    report = build_report(results, cases, started, ctx)
    if args.report_json:
