...
| 元素与定位器 | `feature_element_multi_click` | 主门禁保留当前稳定的多击行为；AX、相对定位、数字开头 id 和显式定位符失败继续由诊断套件复现。 |
...
| 范围 | 用例 |
| --- | --- |
| 公开 API 与启动兼容 | `api_surface`, `feature_recommended_chromium_entry`, `feature_startup_settings` |
| AX 与相对定位 | `feature_element_locators`, `locator_behavior`, `feature_element_core_behaviors`, `feature_find_api`, `feature_regression_basics`, `locator_explicit_syntax` |
| WebSocket/SSE 监听数据包 | `listener_ws_sse`, `feature_listener_upgrade` |
| iframe 内触发的监听数据包 | `feature_cross_origin_iframe_listener` |
| 监听重启回归 | `feature_regression_basics` |
| 标签页管理回归/观察 | `feature_tab_management`, `feature_chromium_tab_management`, `context_get_tabs_as_id` |
| Chromium tab POST 返回值 | `feature_tab_post_response` |
| 初始导航期间出现 alert | `alert_on_open` |
| iframe 非截图访问观察 | `iframe_non_screenshot` |
