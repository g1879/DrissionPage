...
def _check_report_only_steps(path: Path, lines: list[str], errors: list[str]) -> None:
    """Keep explicitly report-only checks from becoming accidental gates."""
    step_start = None
    step_name = ""
    step_lines: list[str] = []

    def check_current() -> None:
        if "report-only" not in step_name.lower():
            return
        for offset, line in enumerate(step_lines):
            if "--fail-on-failures" in line:
                errors.append(
                    f"{path}:{step_start + offset}: report-only step {step_name!r} "
                    "must not use --fail-on-failures"
                )
    ...
    for line_no, line in enumerate(lines, start=1):
        if STEP_START_RE.match(line):
            check_current()
            step_start = line_no
            step_name = line.split("name:", 1)[1].strip() if "name:" in line else ""
            step_lines = [line]
...
def check_workflow(path: Path) -> list[str]:
    ...
    _check_report_only_steps(path, lines, errors)
    return errors
